		...
		switch(mode){
			...
			case 2:
				...
				*****************************************/	
				...
				if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4 || SW_C == SW_ON){
					...
					log_timer = 0;
					len_mouse = 0;
					fast_run(GOAL_X,GOAL_Y,1);		//ゴールまで足立法
					turn(180,TURN_ACCEL,TURN_SPEED,RIGHT);			//ゴールしたら180度回転する
					mypos.dir = (mypos.dir+6) % 4;		//方角を更新
					map_write();
					melody(2);
			...
			// 最短 spd 1.2 acc 2 
				...
				if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4 || SW_C == SW_ON){
					...
					log_timer = 0;
					len_mouse = 0;
					fast_run(GOAL_X,GOAL_Y,2);		//ゴールまで足立法
					turn(180,TURN_ACCEL,TURN_SPEED,RIGHT);			//ゴールしたら180度回転する
					mypos.dir = (mypos.dir+6) % 4;		//方角を更新
					map_write();
					BEEP();
			...
			//最短 spd 1.2 acc 2.2
				...
				if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4 || SW_C == SW_ON){
					...
					log_timer = 0;
					len_mouse = 0;
					fast_run(GOAL_X,GOAL_Y,3);		//ゴールまで足立法
					turn(180,TURN_ACCEL,TURN_SPEED,RIGHT);			//ゴールしたら180度回転する
					mypos.dir = (mypos.dir+6) % 4;		//方角を更新
					map_write();
					BEEP();
			...
			//最短 spd 1.5 acc 2.0
				...
				if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4 || SW_C == SW_ON){
					...
					log_timer = 0;
					len_mouse = 0;
					fast_run(GOAL_X,GOAL_Y,4);		//ゴールまで足立法
					turn(180,TURN_ACCEL,TURN_SPEED,RIGHT);			//ゴールしたら180度回転する
					mypos.dir = (mypos.dir+6) % 4;		//方角を更新
					map_write();
					BEEP();
			...
			//最短　spd 1.5 acc 2.2
				...
				if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4 || SW_C == SW_ON){
					...
					log_timer = 0;
					len_mouse = 0;
					fast_run(GOAL_X,GOAL_Y,5);		//ゴールまで足立法
					turn(180,TURN_ACCEL,TURN_SPEED,RIGHT);			//ゴールしたら180度回転する
					mypos.dir = (mypos.dir+6) % 4;		//方角を更新
					map_write();
					BEEP();
