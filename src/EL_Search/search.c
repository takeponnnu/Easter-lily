...
int check_wall(void);						// 壁判定時の情報をLEDで表示
...
//引数gx,gyに向かって足立法で迷路を探索する
	...
	//if( n == 0){ 
	//straight(HALF_SECTION,SEARCH_ACCEL,SEARCH_SPEED,SEARCH_SPEED); //n++; }
	//len_mouse = 0;
	...
	while((mypos.x != gx) || (mypos.y != gy)){				//ゴールするまで繰り返す
		...
		switch(get_nextdir(gx,gy,MASK_SEARCH,&glob_nextdir))		//次に行く方向を戻り値とする関数を呼ぶ
		{
			...
			case rear:
				//char back_flag;	宣言をラベルすることは出来ないよ、と警告が出るので上で宣言してる。
				back_flag = sen_fr.is_wall | sen_fl.is_wall;
...
int check_wall(void)
{
	int s;
	s = 0;
	if(sen_r.is_wall == true)	s += 8;
	if(sen_l.is_wall == true)	s += 1;
	if(sen_fr.is_wall == true)	s += 4;
	if(sen_fl.is_wall == true)	s += 2;
