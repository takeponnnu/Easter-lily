...
{
	//LED
	PORTC.PDR.BIT.B3 = IO_OUT;//LED0 red
	PORT3.PDR.BIT.B1 = IO_OUT;//LED1
	PORT5.PDR.BIT.B5 = IO_OUT;//LED2
	PORTB.PDR.BIT.B7 = IO_OUT;//LED3
	PORT5.PDR.BIT.B4 = IO_OUT;//LED4
	//PORTA.PDR.BIT.B3 = IO_OUT;//BLED1
	//Sensor
	PORTB.PDR.BIT.B6 = IO_OUT;//SLED_L
	...
	//MOT_CWCCW
	PORTA.PDR.BIT.B0 = 1;//Rmotor PH BIN1
	PORTA.PDR.BIT.B4 = 1;//Rmotor EN BIN2
	PORTB.PDR.BIT.B0 = 1;//Lmotor PH AIN1
	PORTA.PDR.BIT.B6 = 1;//Lmotor EN AIN2
	...
	MPC.P15PFS.BIT.PSEL = 4;	//TCLKB 00100b
...
//	init_sci();			//SCI(Serial Communication Interface)初期化
	...
	preprocess_spi_gyro_2byte(0x6B00);		//スリープモード解除
	for(i = 0; i < 100*1000*10; i++);
	//preprocess_spi_gyro_2byte(0x0601);		//Low Power Mode OFF
	preprocess_spi_gyro_2byte(0x6A10);		//SPIを有効にする
	for(i = 0; i < 100*1000*10; i++);
